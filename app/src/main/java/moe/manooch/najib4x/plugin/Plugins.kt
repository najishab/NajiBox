package moe.manooch.najib4x.plugin
...
import io.najishab.najibox.plugin.PluginManager.loadString
import io.najishab.najibox.utils.PackageCache

object Plugins {
    const val AUTHORITIES_PREFIX_SEKAI_EXE = "io.najishab.najibox.plugin."
    const val AUTHORITIES_PREFIX_NEKO_EXE = "moe.matsuri.exe."

    const val ACTION_NATIVE_PLUGIN = "io.najishab.najibox.plugin.ACTION_NATIVE_PLUGIN"

    const val METADATA_KEY_ID = "io.najishab.najibox.plugin.id"
    const val METADATA_KEY_EXECUTABLE_PATH = "io.najishab.najibox.plugin.executable_path"
    ...
    private fun getExtPluginOld(pluginId: String): List<ProviderInfo> {
        ...
        val list1 = SagerNet.application.packageManager.queryIntentContentProviders(
            Intent(ACTION_NATIVE_PLUGIN, buildUri(pluginId, "io.najishab.najibox")), flags
        )
