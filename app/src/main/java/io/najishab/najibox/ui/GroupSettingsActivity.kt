...
import io.najishab.najibox.widget.OutboundPreference
...
class GroupSettingsActivity(
    ...
    suspend fun saveAndExit() {
        ...
        if (editingId == 0L) {
            val group = ProxyGroup().apply { serialize() }
            GroupManager.createGroup(group)
            if (group.type == GroupType.SUBSCRIPTION && !group.subscription?.link.isNullOrBlank()) {
                GroupUpdater.startUpdate(group, false)
            }
        } else if (needSave()) {
            ...
            if (entity.type == GroupType.SUBSCRIPTION && !entity.subscription?.link.isNullOrBlank()) {
                GroupUpdater.startUpdate(entity, false)
            }
        }
