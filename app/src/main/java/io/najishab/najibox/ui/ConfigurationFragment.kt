...
import io.najishab.najibox.GroupOrder
import io.najishab.najibox.GroupType
...
import io.najishab.najibox.database.GroupManager
...
import io.najishab.najibox.database.SagerDatabase
import io.najishab.najibox.database.preference.OnPreferenceDataStoreChangeListener
import io.najishab.najibox.databinding.LayoutProfileListBinding
import io.najishab.najibox.databinding.LayoutProgressListBinding
import io.najishab.najibox.fmt.AbstractBean
import io.najishab.najibox.fmt.toUniversalLink
import io.najishab.najibox.group.GroupUpdater
import io.najishab.najibox.group.RawUpdater
import io.najishab.najibox.ktx.FixedLinearLayoutManager
import io.najishab.najibox.ktx.Logs
import io.najishab.najibox.ktx.SubscriptionFoundException
import io.najishab.najibox.ktx.alert
import io.najishab.najibox.ktx.app
import io.najishab.najibox.ktx.dp2px
...
import io.najishab.najibox.ktx.runOnLifecycleDispatcher
import io.najishab.najibox.ktx.runOnMainDispatcher
import io.najishab.najibox.ktx.scrollTo
import io.najishab.najibox.ktx.showAllowingStateLoss
import io.najishab.najibox.ktx.snackbar
import io.najishab.najibox.ktx.startFilesForResult
import io.najishab.najibox.ktx.tryToShow
import io.najishab.najibox.plugin.PluginManager
import io.najishab.najibox.ui.profile.ChainSettingsActivity
import io.najishab.najibox.ui.profile.HttpSettingsActivity
...
import io.najishab.najibox.ui.profile.TrojanGoSettingsActivity
import io.najishab.najibox.ui.profile.TrojanSettingsActivity
...
class ConfigurationFragment @JvmOverloads constructor(
    ...
    // For the virtual "All" group, aggregate proxies from every real group instead of one.
    private fun profilesForGroup(groupId: Long): List<ProxyEntity> {
        return if (groupId == ProxyGroup.ALL_GROUP_ID) {
        ...
        when (item.itemId) {
            ...
            R.id.action_clear_traffic_statistics -> {
                runOnDefaultDispatcher {
                    val profiles = profilesForGroup(DataStore.currentGroupId())
    ...
    inner class TestDialog {
        ...
        // Persist the result of this single profile immediately (instead of waiting
        // for the whole batch to finish), so the profile list reflects it right away.
        suspend fun update(profile: ProxyEntity) {
            ...
            runOnMainDispatcher {
                if (context == null) return@runOnMainDispatcher
                val percent = if (proxyN > 0) progress * 100 / proxyN else 0
                binding.progressBar.setProgressCompat(percent, true)
                binding.progress.text = "$progress / $proxyN ($percent%)"
            }
        }
    ...
    fun pingTest(icmpPing: Boolean) {
        ...
        val mainJob = runOnDefaultDispatcher {
            val profilesList = profilesForGroup(group.id).filter {
                if (icmpPing) {
    ...
    fun urlTest() {
        ...
        val mainJob = runOnDefaultDispatcher {
            val profilesList = profilesForGroup(group.id)
            test.proxyN = profilesList.size
            val profiles = ConcurrentLinkedQueue(profilesList)
            repeat(DataStore.connectionTestConcurrent) {
                testJobs.add(launch(Dispatchers.IO) {
    ...
    inner class GroupPagerAdapter : FragmentStateAdapter(this),
        ...
        fun reload(now: Boolean = false) {
            ...
            runOnDefaultDispatcher {
                ...
                // Prepend the virtual "All" group, aggregating every proxy from every group.
                newGroupList.add(0, ProxyGroup.allGroup())
    ...
    class GroupFragment : Fragment() {
        ...
        override fun onViewStateRestored(savedInstanceState: Bundle?) {
            ...
            if (savedInstanceState != null) {
                BundleCompat.getParcelable(savedInstanceState, "proxyGroup", ProxyGroup::class.java)?.let {
                    proxyGroup = it
            ...
            if (!select) {
                ...
                ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(
                    ...
                    override fun getDragDirs(
                        recyclerView: RecyclerView,
                        viewHolder: RecyclerView.ViewHolder,
                    ) = if (isEnabled && proxyGroup.id != ProxyGroup.ALL_GROUP_ID) super.getDragDirs(recyclerView, viewHolder) else 0
        ...
        inner class ConfigurationAdapter : RecyclerView.Adapter<ConfigurationHolder>(),
            ...
            override suspend fun onAdd(profile: ProxyEntity) {
                if (proxyGroup.id != ProxyGroup.ALL_GROUP_ID && profile.groupId != proxyGroup.id) return
            ...
            override suspend fun onRemoved(groupId: Long, profileId: Long) {
                if (proxyGroup.id != ProxyGroup.ALL_GROUP_ID && groupId != proxyGroup.id) return
            ...
            fun reloadProfiles() {
                var newProfiles = if (proxyGroup.id == ProxyGroup.ALL_GROUP_ID) {
                    SagerDatabase.proxyDao.getAll()
                } else {
