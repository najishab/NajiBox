...
import io.najishab.najibox.databinding.LayoutGroupItemBinding
...
import moe.manooch.najib4x.utils.toBytesString
                    ...
                    @Suppress("DEPRECATION") when (group.type) {
                        ...
                        GroupType.SUBSCRIPTION -> {
                            groupStatus.text = if (size == 0L) {
                                ...
                                val currentLocale = ConfigurationCompat.getLocales(requireContext().resources.configuration)[0]
                                    ?: Locale.getDefault()
                                val formattedDate = if (currentLocale.language == "fa") {
                                    Util.getPersianDate(date)
                                } else {
                                    SimpleDateFormat("dd/MM/yyyy", currentLocale).format(date)
                                }
                                getString(
